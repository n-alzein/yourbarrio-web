...
import { restoreInventoryForOrder } from "@/lib/orders/inventoryReservations";
import { commitOrderInventoryFromCartReservations } from "@/lib/cart/reservations";
...
async function triggerNewOrderNotifications({
  client,
  orderId,
  nextStatus,
  source,
  ...
  if (!orderId) {
    console.info(logPrefix, "send_new_order_notifications_skipped", {
      source,
      reason: "missing_order_id",
    });
    return { action: "skipped_missing_order_id" };
  }
  ...
  if (nextStatus !== "requested") {
    console.info(logPrefix, "send_new_order_notifications_skipped", {
      source,
      orderId,
      reason: "non_requested_status",
      nextStatus: nextStatus || null,
    });
    return { action: "skipped_non_requested_status" };
  }

  console.info(logPrefix, "send_new_order_notifications_start", {
    source,
    orderId,
    nextStatus,
  });
  ...
  try {
    const result = await sendNewOrderNotifications(orderId, { client });
    console.info(logPrefix, "send_new_order_notifications_success", {
      source,
      orderId,
      nextStatus,
    ...
    console.error(logPrefix, "send_new_order_notifications_failed", {
      source,
      orderId,
      nextStatus,
...
export async function createOrderWithItems({
  ...
  const orderItems = Array.isArray(items)
    ? items.map((item) => ({
        ...
        inventory_reserved_at: item.inventory_reserved_at || null,
  ...
  if (orderItems.length > 0) {
    const { data: insertedItems, error: itemsError } = await client
      .from("order_items")
      .insert(orderItems)
      .select("id,order_id,listing_id,quantity,inventory_reservation_id");
    if (itemsError) {
    ...
    for (const item of insertedItems || []) {
      if (!item?.inventory_reservation_id) continue;
      const { data: attachResult, error: attachError } = await client.rpc(
        "attach_inventory_reservation_to_order_item",
        {
          p_reservation_id: item.inventory_reservation_id,
...
export async function findOrderByStripeContext(client, options = {}) {
  if (options.orderId) {
    const { data } = await client
      .from("orders")
      .select("id,order_number,status,paid_at,vendor_id,user_id,cart_id,inventory_reserved_at,inventory_restored_at")
...
export async function restoreReservedInventoryForOrder({
  client,
  order,
  logPrefix = "[ORDER_FINALIZATION_TRACE]",
  source = "unknown",
}) {
  if (!order?.id || !order.inventory_reserved_at || order.inventory_restored_at) {
    return { action: "skipped" };
  }

  const restored = await restoreInventoryForOrder({ client, orderId: order.id });

  const restoredAt = new Date().toISOString();
  const { error: updateError } = await client
    .from("orders")
    .update({
      inventory_restored_at: restoredAt,
  ...
  if (updateError) {
    throw new Error(updateError.message || "Failed to mark inventory restored");
  }

  logTrace(logPrefix, "inventory_restored", {
    source,
    orderId: order.id,
    orderNumber: order.order_number || null,
    listingIds: restored.map((item) => item.listing_id).filter(Boolean),
  });
...
function buildPaidOrderUpdates({ order, session, paymentIntent }) {
  ...
  const subtotalCents = Number(
    session?.metadata?.subtotal_cents || paymentIntent?.metadata?.subtotal_cents || NaN
  );
  const deliveryFeeCents = Number(
    session?.metadata?.delivery_fee_cents || paymentIntent?.metadata?.delivery_fee_cents || NaN
  );
  ...
  if (Number.isFinite(totalCents) && totalCents >= 0) {
    ...
    if (Number.isFinite(subtotalCents) && subtotalCents >= 0) {
      updates.subtotal = centsToDollars(subtotalCents);
    } else if (Number.isFinite(platformFeeAmount) && platformFeeAmount >= 0) {
  ...
  if (Number.isFinite(deliveryFeeCents) && deliveryFeeCents >= 0) {
    updates.delivery_fee_cents_snapshot = Math.round(deliveryFeeCents);
  }
...
export async function finalizePaidOrderFromCheckoutSession({
  ...
  if (nextStatus === "requested" && order.cart_id) {
  ...
  await triggerNewOrderNotifications({
    client,
    orderId: order.id,
    nextStatus,
    source: "finalizePaidOrderFromCheckoutSession",
    logPrefix,
  });
...
export async function finalizePaidOrderFromPaymentIntent({
  ...
  await triggerNewOrderNotifications({
    ...
    orderId: order.id,
    nextStatus,
    source: "finalizePaidOrderFromPaymentIntent",
    logPrefix,
  });
...
export async function markStripePaymentFailed({
  ...
  // We do not restore inventory on payment_intent.payment_failed because Stripe
  // Checkout can allow another payment attempt for the same reserved order.
  // Stock is released only on local checkout setup failure or explicit order
  // cancellation, where the order can no longer complete unexpectedly.
...
export async function expireStripeCheckoutSession({
  ...
  if (!order?.id) {
    logTrace(logPrefix, "checkout_expired_missing_order", {
      sessionId: session?.id || null,
    });
    return { order: null, action: "missing" };
  }
  ...
  if (order.status !== "pending_payment" && order.status !== "payment_failed") {
    return { order, action: "skipped", nextStatus: order.status || null };
  }

  await restoreReservedInventoryForOrder({
    client,
    order,
    logPrefix,
  ...
  if (error) {
    throw new Error(error.message || "Failed to expire checkout order");
  }

  return { order, action: "expired", nextStatus: "payment_failed" };
}
