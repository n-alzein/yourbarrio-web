...
export function resolveListingCategory(input) {
  ...
  const relationName = trimValue(record?.category_info?.name);
  const relationMatch = getListingCategory(relationName);
  if (relationMatch) {
    return {
      slug: relationMatch.slug,
      label: relationMatch.name,
  ...
  if (legacyMatch) {
    return {
      slug: legacyMatch.slug,
      label: legacyMatch.name,
      source: "legacy_category",
    };
  }

  const raw = trimValue(directValue) || relationName || legacyValue;
  if (!raw) return null;

  return {
    slug: slugifyCategoryName(raw),
    label: normalizeCategoryName(raw),
    source: legacyValue ? "legacy_category" : "listing_category",
  };
}
...
export function buildListingTaxonomyPayload(input = {}) {
  ...
  return {
    listing_category: resolved?.label || trimValue(input.listing_category),
    category: resolved?.label || trimValue(input.category),
