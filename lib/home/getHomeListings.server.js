...
export async function getHomeListings({ limit = 80, location, category } = {}) {
  ...
  let query = supabase
    ...
    .select(
      "id,title,price,category,category_id,city,photo_url,business_id,created_at,inventory_status,inventory_quantity,low_stock_threshold,inventory_last_updated_at,is_seeded,business_is_seeded"
    )
