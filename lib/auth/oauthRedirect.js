...
function isYourBarrioAppHost(hostname) {
  return hostname === "yourbarrio.com" || hostname === "www.yourbarrio.com";
}

function canonicalizeYourBarrioOrigin(origin) {
  const normalized = normalizeOrigin(origin);
  if (!normalized) return "";
  try {
    const url = new URL(normalized);
    if (isYourBarrioAppHost(url.hostname)) {
...
function shouldUseConfiguredCanonical(currentOrigin, configuredOrigin) {
  ...
  try {
    ...
    return isYourBarrioAppHost(current.hostname) && isYourBarrioAppHost(configured.hostname);
  } catch {
    return false;
  }
}
...
function getYourBarrioCanonicalOrigin(currentOrigin) {
  try {
    ...
    if (isYourBarrioAppHost(current.hostname)) {
      return "https://yourbarrio.com";
    }
  } catch {
    // fall through
...
export function getOAuthRedirectOrigin(currentOrigin) {
  ...
  if (shouldUseConfiguredCanonical(detectedOrigin, configuredOrigin)) {
    return canonicalizeYourBarrioOrigin(configuredOrigin);
  }
  return (
    getYourBarrioCanonicalOrigin(detectedOrigin) ||
    detectedOrigin ||
    canonicalizeYourBarrioOrigin(configuredOrigin) ||
    "http://localhost:3000"
  );
}
...
export function getOAuthConfigDiagnostics({ currentOrigin } = {}) {
  ...
  return {
    ...
    configuredSiteUrl: canonicalizeYourBarrioOrigin(configuredSiteUrl) || null,
