...
export const LISTING_FULFILLMENT_MODES = {
  PICKUP: "pickup",
  DELIVERY: "delivery",
  BOTH: "both",
};
...
export function getFulfillmentModeFromBooleans(pickupEnabled, localDeliveryEnabled) {
  if (pickupEnabled && localDeliveryEnabled) {
    return LISTING_FULFILLMENT_MODES.BOTH;
  }

  if (localDeliveryEnabled) {
    return LISTING_FULFILLMENT_MODES.DELIVERY;
  }

  return LISTING_FULFILLMENT_MODES.PICKUP;
}

export function applyFulfillmentModeToForm(form, mode) {
  const normalizedMode = String(mode || "").trim().toLowerCase();
  const nextForm = {
    ...form,
  };

  if (normalizedMode === LISTING_FULFILLMENT_MODES.DELIVERY) {
    nextForm.pickupEnabled = false;
    nextForm.localDeliveryEnabled = true;
    return nextForm;
  }

  if (normalizedMode === LISTING_FULFILLMENT_MODES.BOTH) {
    nextForm.pickupEnabled = true;
    nextForm.localDeliveryEnabled = true;
    return nextForm;
  }

  nextForm.pickupEnabled = true;
  nextForm.localDeliveryEnabled = false;
  nextForm.useBusinessDeliveryDefaults = true;
  return nextForm;
}
...
export function buildListingDraftData({
  ...
  return {
    ...
    listing_category_id: taxonomy?.listing_category_id || null,
    category_id: null,
    city: form?.city || "",
...
export function applyListingDraftDataToListing(listing, draftData) {
  ...
  return {
    listing: {
      ...
      ...(listing_category_id !== undefined ? { listing_category_id } : {}),
